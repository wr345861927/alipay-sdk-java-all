...
public class AlipayCommerceWithholdrepayorderPaySignModel extends AlipayObject {
	...
	/**
	 * 禁用渠道,用户不可用指定渠道支付，多个渠道以逗号分割 注，与enable_pay_channels互斥 渠道列表
	 */
	@ApiField("disable_pay_channels")
	private String disablePayChannels;

	/**
	 * 指定支付渠道，多个渠道以逗号分割。用户只能使用此处指定渠道进行支付。 与disable_pay_channels互斥，支持传入的值：渠道列表。 注意：如果传入了指定支付渠道，则用户只能用指定内的渠道支付，包括营销渠道也要指定才能使用。若所有指定渠道用户都不可使用，将导致用户无法支付，慎用。
	 */
	...
	private OpenInvoiceInfo invoiceInfo;
	...
	/**
	 * 产品码。 商家和支付宝签约的产品码。 枚举值（点击查看签约情况）： QUICK_MSECURITY_PAY：无线快捷支付产品； CYCLE_PAY_AUTH：周期扣款产品。 默认值为QUICK_MSECURITY_PAY。
	 */
	...
	public SignParam getAgreementSignParams() {
		return this.agreementSignParams;
	}
	public void setAgreementSignParams(SignParam agreementSignParams) {
	...
	public OpenInvoiceInfo getInvoiceInfo() {
		return this.invoiceInfo;
	}
	public void setInvoiceInfo(OpenInvoiceInfo invoiceInfo) {
