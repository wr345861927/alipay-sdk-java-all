...
import com.alipay.api.response.AlipayCommerceTransportTrafficshareTradeRefundResponse;
 ...
 */
public class AlipayCommerceTransportTrafficshareTradeRefundRequest implements AlipayRequest<AlipayCommerceTransportTrafficshareTradeRefundResponse> {
	...
	* 当交易发生之后一段时间内，由于买家或者卖家的原因需要退款时，卖家可以通过退款接口将支付款退还给买家，支付宝将在收到退款请求并且验证成功之后，按照退款规则将支付款按原路退到买家账号上。 交易超过约定时间（签约时设置的可退款时间）的订单无法进行退款。 支付宝退款支持单笔交易分多次退款，多次退款需要提交原支付订单的订单号和设置不同的退款请求号。一笔退款失败后重新提交，要保证重试时退款请求号不能变更，防止该笔交易重复退款。 同一笔交易累计提交的退款金额不能超过原始交易总金额。
	...
	public Class<AlipayCommerceTransportTrafficshareTradeRefundResponse> getResponseClass() {
		return AlipayCommerceTransportTrafficshareTradeRefundResponse.class;
	}
